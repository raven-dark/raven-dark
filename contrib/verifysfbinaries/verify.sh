...
###   This script attempts to download the signature file SHA256SUMS.asc from raven-dark.com
...
WORKINGDIR="/tmp/ravendark"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://raven-dark.com/bin/"
VERSIONPREFIX="ravendark-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'ravendark-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the RavenDark Core binary release signing key installed?"
   fi
