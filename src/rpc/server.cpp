...
UniValue stop(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "\nStop RavenDark Core server.");
    // Event loop will exit after current HTTP requests have been handled, so
    // this reply will get back to the client.
    StartShutdown();
    return "RavenDark Core server stopping";
}
...
{ //  category              name                      actor (function)         okSafeMode
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    { "ravendark",               "voteraw",                &voteraw,                true  },
    { "ravendark",               "mnsync",                 &mnsync,                 true  },
    { "ravendark",               "spork",                  &spork,                  true  },
    { "ravendark",               "getpoolinfo",            &getpoolinfo,            true  },
    { "ravendark",               "sentinelping",           &sentinelping,           true  },
#ifdef ENABLE_WALLET
    { "ravendark",               "privatesend",            &privatesend,            false },
...
{
    return "> ravendark-cli " + methodname + " " + args + "\n";
}
