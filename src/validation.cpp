...
#if defined(NDEBUG)
# error "RavenDark Core cannot be compiled without assertions."
#endif
                              ...
                              std::vector<COutPoint>& coins_to_uncache, bool fDryRun)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s XRD additional fees, %d delta bytes\n",
...
{
    int year = consensusParams.nSubsidyHalvingInterval;
    CAmount nSubsidy = 200 * COIN / 100;

    for (int i = year; i <= nPrevHeight; i += year) {
	nSubsidy = (CAmount)(nSubsidy * 0.5);
    }
    nSubsidy = nSubsidy * 100;
    //When 17395.00 days later, the block height > 12524400, the largest supply of XRD is 210000000.00000000.
...
{
    // Until tiers implementation, MN gets 15% from block reward
...
void ThreadScriptCheck() {
    RenameThread("ravendark-scriptch");
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    // RAVENDARK : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(RAVENDARK): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(RAVENDARK): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // RAVENDARK : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(RAVENDARK): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(RAVENDARK): spork is off, skipping transaction locking checks\n");
    }
