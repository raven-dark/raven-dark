...
const char * const RAVENDARK_CONF_FILENAME = "ravendark.conf";
const char * const RAVENDARK_PID_FILENAME = "ravendarkd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "ravendark" is a composite category enabling all RavenDark-related debug output
            if(ptrCategory->count(string("ravendark"))) {
...
#else
    const char* pszModule = "ravendark";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\RavenDarkCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\RavenDarkCore
    // Mac: ~/Library/Application Support/RavenDarkCore
    // Unix: ~/.ravendarkcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "RavenDarkCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/RavenDarkCore";
#else
    // Unix
    return pathRet / ".ravendarkcore";
#endif
#endif
}
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", RAVENDARK_CONF_FILENAME));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty ravendark.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override ravendark.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", RAVENDARK_PID_FILENAME));
