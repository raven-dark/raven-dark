...
CBlockTemplate* CreateNewBlock(const CChainParams& chainparams, const CScript& scriptPubKeyIn)
{
...

    {
        ...
        // NOTE: unlike in ravendark, we need to pass PREVIOUS block height here
...
// ***TODO*** ScanHash is not yet used in RavenDark
//
...
// ***TODO*** that part changed in ravendark, we are using a mix with old one here for now
void static RavenDarkMiner(const CChainParams& chainparams, CConnman& connman)
{
    LogPrintf("RavenDarkMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ravendark-miner");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("RavenDarkMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("RavenDarkMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("RavenDarkMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("RavenDarkMiner -- runtime error: %s\n", e.what());
        return;
    }
}

void GenerateRavenDarks(bool fGenerate, int nThreads, const CChainParams& chainparams, CConnman& connman)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RavenDarkMiner, boost::cref(chainparams), boost::ref(connman)));
}
