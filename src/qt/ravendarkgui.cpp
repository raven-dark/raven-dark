...
const std::string RavenDarkGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString RavenDarkGUI::DEFAULT_WALLET = "~Default";

RavenDarkGUI::RavenDarkGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
    ...
    platformStyle(platformStyle)
{
    ...
    QPixmap pixmapLogo(":/images/light/ravendark_logo_horizontal");
...
void RavenDarkGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a RavenDark address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and ravendark: URIs)"));
    ...
    aboutAction = new QAction(tr("&About RavenDark"), this);
    aboutAction->setStatusTip(tr("Show information about RavenDark"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for RavenDark"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your RavenDark addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified RavenDark addresses"));
    ...
    openAction->setStatusTip(tr("Open a ravendark: URI or payment request"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the RavenDark help message to get a list with possible RavenDark command-line options"));
...
void RavenDarkGUI::createToolBars(QWidget* statusbar)
{
    if(walletFrame)
    {
...
void RavenDarkGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool RavenDarkGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool RavenDarkGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void RavenDarkGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void RavenDarkGUI::setWalletActionsEnabled(bool enabled)
{
...
void RavenDarkGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
    trayIcon = new QSystemTrayIcon(this);
    QString toolTip = tr("RavenDark client") + " " + networkStyle->getTitleAddText();
...
void RavenDarkGUI::createIconMenu(QMenu *pmenu)
{
...
#ifndef Q_OS_MAC
void RavenDarkGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void RavenDarkGUI::optionsClicked()
{
...
void RavenDarkGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void RavenDarkGUI::showDebugWindow()
{
...
void RavenDarkGUI::showInfo()
{
    toolsAction->setChecked(true);
    walletFrame->gotoToolsPageTab(ToolsPage::TAB_INFO);
}

void RavenDarkGUI::showConsole()
{
    toolsAction->setChecked(true);
    walletFrame->gotoToolsPageTab(ToolsPage::TAB_CONSOLE);
}

void RavenDarkGUI::showGraph()
{
    toolsAction->setChecked(true);
    walletFrame->gotoToolsPageTab(ToolsPage::TAB_GRAPH);
}

void RavenDarkGUI::showPeers()
{
    toolsAction->setChecked(true);
    walletFrame->gotoToolsPageTab(ToolsPage::TAB_PEERS);
}

void RavenDarkGUI::showRepair()
{
    toolsAction->setChecked(true);
    walletFrame->gotoToolsPageTab(ToolsPage::TAB_REPAIR);
}

void RavenDarkGUI::showConfEditor()
{
    GUIUtil::openConfigfile();
}

void RavenDarkGUI::showMNConfEditor()
{
    GUIUtil::openMNConfigfile();
}

void RavenDarkGUI::showBackups()
{
    GUIUtil::showBackups();
}

void RavenDarkGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

void RavenDarkGUI::showPrivateSendHelpClicked()
{
    if(!clientModel)
        return;
...
void RavenDarkGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void RavenDarkGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void RavenDarkGUI::gotoSettingsPage() 
{
    settingsAction->setChecked(true);
    if(walletFrame) walletFrame->gotoSettingsPage();
}

void RavenDarkGUI::gotoToolsPage()
{
    toolsAction->setChecked(true);
    if(toolsAction) walletFrame->gotoToolsPage();
}

void RavenDarkGUI::gotoMasternodePage()
{
    QSettings settings;
    if (settings.value("fShowMasternodesTab").toBool()) {
...
void RavenDarkGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void RavenDarkGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void RavenDarkGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void RavenDarkGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void RavenDarkGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        labelConnectionsIcon->setToolTip(tr("%n active connection(s) to RavenDark network", "", count));
    } else {
...
void RavenDarkGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void RavenDarkGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void RavenDarkGUI::updateHeadersSyncProgressLabel()
{
...
void RavenDarkGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
    ...
    if (walletFrame)
    {
        if(secs < 25*60) // 90*60 in ravendark
        {
...
void RavenDarkGUI::setAdditionalDataSyncProgress(double nSyncProgress)
{
    if(!clientModel)
        return;
...
void RavenDarkGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("RavenDark"); // default title
...
void RavenDarkGUI::changeEvent(QEvent *e)
{
...
void RavenDarkGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void RavenDarkGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void RavenDarkGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(RavenDarkUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void RavenDarkGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void RavenDarkGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool RavenDarkGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool RavenDarkGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void RavenDarkGUI::setHDStatus(int hdEnabled)
{
    QString theme = GUIUtil::getThemeName();
...
void RavenDarkGUI::setEncryptionStatus(int status)
{
    QString theme = GUIUtil::getThemeName();
    switch(status)
    {
...
void RavenDarkGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void RavenDarkGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void RavenDarkGUI::detectShutdown()
{
...
void RavenDarkGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void RavenDarkGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void RavenDarkGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(RavenDarkGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void RavenDarkGUI::subscribeToCoreSignals()
{
...
void RavenDarkGUI::unsubscribeFromCoreSignals()
{
...
void RavenDarkGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void RavenDarkGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<RavenDarkUnits::Unit> units = RavenDarkUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const RavenDarkUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(RavenDarkUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    Q_FOREACH(RavenDarkUnits::Unit u, RavenDarkUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(RavenDarkUnits::name(u)), this);
...
{
    setText(RavenDarkUnits::name(newUnits));
}
