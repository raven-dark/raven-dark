    ...
    ui(new Ui::HelpMessageDialog)
{
    ...
    QString version = tr("RavenDark Core") + " " + tr("version") + " " + QString::fromStdString(FormatFullVersion());
    ...
    if (helpMode == about)
    {
        setWindowTitle(tr("About RavenDark Core"));
        ...
        QString header = tr("Usage:") + "\n" +
            "  ravendark-qt [" + tr("command-line options") + "]                     " + "\n";
        ...
        std::string strUsage = HelpMessage(HMM_RAVENDARK_QT);
        ...
        if (showDebug) {
            strUsage += HelpMessageOpt("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", RavenDarkGUI::DEFAULT_UIPLATFORM));
        }
...
All the RavenDark in your wallet is comprised of different \"inputs\" which you can think of as separate, discrete coins.<br> \
...
These denominations are 0.01 RAVENDARK, 0.1 RAVENDARK, 1 RAVENDARK and 10 RAVENDARK -- sort of like the paper money you use every day.</li> \
    ...
    QWidget(parent, f)
{
    ...
    layout->addWidget(new QLabel(
        tr("RavenDark Core is shutting down...") + "<br /><br />" +
        tr("Do not shut down the computer until this window disappears.")));
    setLayout(layout);
}

QWidget *ShutdownWindow::showShutdownWindow(RavenDarkGUI *window)
{
    if (!window)
        return nullptr;
